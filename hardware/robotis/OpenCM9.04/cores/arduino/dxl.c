...
void dxlInterrupt1(byte data){
	...
	//TxDStringC("test");
	DXL_DEV1->data_buffer[DXL_DEV1->write_pointer++ & DXL_RX_BUF_SIZE] = data;
	//mDxlDevice->read_pointer = mDxlDevice->read_pointer & DXL_RX_BUF_SIZE;
}
void dxlInterrupt2(byte data){
	//gbpDXLDataBuffer[gbDXLWritePointer++ & 0xFF] = (uint8)USART1->regs->DR; //[ROBOTIS]Support to Dynamixel SDK.
	//TxDStringC("test");

	DXL_DEV2->data_buffer[DXL_DEV2->write_pointer++ & DXL_RX_BUF_SIZE] = data;
	//mDxlDevice->read_pointer = mDxlDevice->read_pointer & DXL_RX_BUF_SIZE;
}
void dxlInterrupt3(byte data){
	//gbpDXLDataBuffer[gbDXLWritePointer++ & 0xFF] = (uint8)USART1->regs->DR; //[ROBOTIS]Support to Dynamixel SDK.
	//TxDHex8C(data);
	DXL_DEV3->data_buffer[DXL_DEV3->write_pointer++ & DXL_RX_BUF_SIZE] = data;
}
